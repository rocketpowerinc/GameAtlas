 ...
 if(smoke){
  try{
   ...
   await window.webContents.executeJavaScript(`(async()=>{
    ...
    const sourceEditor=document.querySelector('.source-editor-grid'),sourceIds=[...sourceEditor.querySelectorAll('input')].map(input=>input.id);if(JSON.stringify(sourceIds)!==JSON.stringify(['edit-source-youtube','edit-source-ign','edit-source-steam','edit-source-wikipedia','edit-source-howlongtobeat'])||getComputedStyle(sourceEditor).gridTemplateColumns.split(' ').length!==1)throw Error('Source editors are not ordered in full-width rows');
   ...
   await window.webContents.executeJavaScript(`(async()=>{
    ...
    const ign=document.querySelector('#edit-source-ign'),wiki=document.querySelector('#edit-source-wikipedia'),hltb=document.querySelector('#edit-source-howlongtobeat'),youtube=document.querySelector('#edit-source-youtube'),input=Object.getOwnPropertyDescriptor(HTMLInputElement.prototype,'value').set;
    ...
    const sourceLabels=[...document.querySelectorAll('.game-page-source-links a')].map(link=>link.textContent.trim());if(JSON.stringify(sourceLabels)!==JSON.stringify(['YouTube','Steam','Wikipedia','HowLongToBeat']))throw Error('Game-page sources are not in the required order');
