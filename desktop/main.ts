 ...
 if(smoke){
  try{
   ...
   await window.webContents.executeJavaScript(`(async()=>{
    ...
    if(document.querySelector('#edit-source-ign')?.value!=='https://www.ign.com/games/test-game'||document.querySelector('#edit-source-steam')?.value!=='https://store.steampowered.com/app/2'||!document.querySelector('#edit-source-wikipedia')||!document.querySelector('#edit-source-howlongtobeat'))throw Error('Source URL editors missing');
   })()`);
   ...
   await window.webContents.executeJavaScript(`(async()=>{
    ...
    const ign=document.querySelector('#edit-source-ign'),wiki=document.querySelector('#edit-source-wikipedia'),hltb=document.querySelector('#edit-source-howlongtobeat'),input=Object.getOwnPropertyDescriptor(HTMLInputElement.prototype,'value').set;
    input.call(ign,'');ign.dispatchEvent(new Event('input',{bubbles:true}));input.call(wiki,'https://en.wikipedia.org/wiki/Test_Game');wiki.dispatchEvent(new Event('input',{bubbles:true}));input.call(hltb,'https://howlongtobeat.com/game/1');hltb.dispatchEvent(new Event('input',{bubbles:true}));await new Promise(r=>setTimeout(r,100));
    ...
    if(document.querySelector('.game-page-source-links')?.textContent.includes('IGN')||!document.querySelector('.game-page-source-links')?.textContent.includes('Steam')||!document.querySelector('.game-page-source-links')?.textContent.includes('Wikipedia')||!document.querySelector('.game-page-source-links')?.textContent.includes('HowLongToBeat'))throw Error('Edited sources were not saved');
