 ...
 if(smoke){
  try{
   ...
   await window.webContents.executeJavaScript(`(async()=>{
    ...
    if(!compact||!compact.classList.contains('score-great')||compact.querySelector('h2')?.textContent!=='Test game'||compact.querySelector('.game-info')||compact.querySelector('.card-bottom')||compact.querySelectorAll('.game-badge-icon').length!==2||compact.querySelector('.compact-card-art .game-card-badges')||compactBadges.getBoundingClientRect().top<compactReplay.getBoundingClientRect().bottom||compactReplay.textContent.trim()!=='Replay OnSTPS5'||compactReplay.querySelectorAll('.replay-platform-icon').length!==2)throw Error('Compact card is not artwork-and-title only or lost its replay and badge rows');
    ...
    if([...playNextChoices.querySelectorAll('[data-slot="checkbox"]')].filter(toggle=>toggle.getAttribute('aria-checked')==='true').length!==2)throw Error('Play Next On does not preserve multiple selections');
    ...
    if(filters.length!==7||document.querySelector('[aria-label="Filter play next on"]')||filters.some(filter=>filter.top<search.bottom)||Math.max(...filters.map(filter=>filter.width))-Math.min(...filters.map(filter=>filter.width))>2)throw Error('Search and filter layout is uneven or destination filter remains');
