...
app.whenReady().then(async()=>{
 ...
 ipcMain.handle('export-backup',async event=>{trusted(event);const {filePath}=await dialog.showSaveDialog(window,{defaultPath:'gameatlas-backup-'+new Date().toISOString().slice(0,10)+'.json',filters:[{name:'GameAtlas backup',extensions:['json']}]});if(filePath)writeFileSync(filePath,JSON.stringify({format:'gameatlas-v1',...store.read()},null,2));return !!filePath;});
 ipcMain.handle('backup-folder',async event=>{trusted(event);const result=await dialog.showOpenDialog(window,{properties:['openDirectory','createDirectory']});if(result.canceled)return false;writeFileSync(join(app.getPath('userData'),'settings.json'),JSON.stringify({backupFolder:result.filePaths[0]}));mirror();return true;});
