 ...
 if(smoke){
  try{
   ...
   await window.webContents.executeJavaScript(`(async()=>{
    ...
    const playNextChoices=document.querySelector('[aria-label="Play Next On"]');if(!playNextChoices||[...playNextChoices.querySelectorAll('.choice')].slice(0,4).map(choice=>choice.textContent.trim()).join('|')!=='Steam|Switch 2|Playstation 5|Xbox Series X')throw Error('Play Next On choices missing or out of order');
    const ps5Choice=[...playNextChoices.querySelectorAll('.choice')].find(choice=>choice.textContent.trim()==='Playstation 5'),ps5Toggle=ps5Choice?.querySelector('[data-slot="checkbox"]');ps5Toggle?.click();await wait();if(ps5Toggle?.getAttribute('aria-checked')!=='true')throw Error('Play Next On does not allow multiple selections');
    ...
    if(filters.length!==8||filters.some(filter=>filter.top<search.bottom)||Math.max(...filters.map(filter=>filter.width))-Math.min(...filters.map(filter=>filter.width))>2)throw Error('Search and filter layout is uneven');
    ...
    const playNextFilter=document.querySelector('[aria-label="Filter play next on"]');playNextFilter.click();await wait();
    const steam=[...document.querySelectorAll('[role="option"]')].find(option=>option.textContent.trim()==='Steam');if(!steam)throw Error('Play Next On filter missing');steam.click();await wait();
    if(!playNextFilter.textContent.includes('Steam')||document.querySelectorAll('.game-card').length!==1)throw Error('Play Next On filter did not select matching games');
