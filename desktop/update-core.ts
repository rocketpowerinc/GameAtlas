...
export type UpdateFlavor='setup'|'portable';
export type UpdateRelease={version:string;url:string;size:number;digest:string;notes?:string;flavor:UpdateFlavor};
...
export function selectRelease(raw:any,current:string,flavor:UpdateFlavor='setup'):UpdateRelease|null{
 ...
 const label=flavor==='portable'?'Portable':'Setup';
 const filename=new RegExp('^GameAtlas[ .]'+label+'[ .]'+version.replaceAll('.','\\.')+'\\.exe$','i');
 const asset=Array.isArray(raw.assets)?raw.assets.find((a:any)=>filename.test(a.name)&&a.state==='uploaded'):null;
 if(!asset||!Number.isSafeInteger(asset.id)||asset.id<=0||!Number.isSafeInteger(asset.size)||asset.size<1||asset.size>400_000_000||!/^sha256:[a-f0-9]{64}$/.test(asset.digest||''))throw Error('The new release does not have a verified Windows '+(flavor==='portable'?'portable executable':'installer')+' yet. Try again later.');
 return {version,notes:typeof raw.body==='string'?raw.body.trim().slice(0,50000):'',url:repository+'/releases/assets/'+asset.id,size:asset.size,digest:asset.digest.slice(7),flavor};
}
const headers=(accept:string)=>({'User-Agent':'GameAtlas-Windows-Updater',Accept:accept,'X-GitHub-Api-Version':'2022-11-28'});
export async function latest(current:string,request:typeof fetch=fetch,flavor:UpdateFlavor='setup'){
 ...
 return selectRelease(JSON.parse(text),current,flavor);
}
export async function download(release:UpdateRelease,directory:string,progress:(n:number)=>void=()=>{},request:typeof fetch=fetch){
 ...
 try{
  ...
  if(!response?.ok||!response.body)throw Error('The update could not be downloaded. Try again.');
  const hash=createHash('sha256');let size=0,last=-1;
  const verify=new Transform({transform(chunk,encoding,callback){
   size+=chunk.length;if(size>release.size){callback(Error('The downloaded update is larger than expected.'));return;}
  ...
  if(size!==release.size||hash.digest('hex')!==release.digest)throw Error('The update failed its integrity check. It has been discarded.');
