...
export function collectionPdfHtml(library:Library,directory:string,created=new Date(),subtitle='Entire Library Catalog'){
 ...
 const consoles=hardware.filter(item=>item.type==='Console'),peripherals=hardware.filter(item=>item.type==='Peripheral'),unitCount=(items:typeof hardware)=>items.reduce((total,item)=>total+(item.quantity??1),0);
