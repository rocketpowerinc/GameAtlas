...
export function collectionPdfHtml(library:Library,directory:string,created=new Date(),subtitle='Entire Library Catalog'){
 ...
 const hardware=[...(library.hardware??[])].sort((a,b)=>a.type.localeCompare(b.type)||a.name.localeCompare(b.name,undefined,{sensitivity:'base'}));
 ...
 const hardwareCards=hardware.map((item,index)=>{const image=art(item.coverUrl,directory);return `<article class="game hardware-item"><div class="number">${index+1}</div>${image?`<img src="${image}" alt="">`:'<div class="cover">HW</div>'}<section><h2>${escape(item.name)}</h2><p class="sub">${escape([item.manufacturer,item.model].filter(Boolean).join(' - ')||item.type)}</p><div class="facts">${line('Type',item.type)}${line('Released',item.releaseDate||'')}${line('Model',item.model||'')}</div>${item.description?`<p class="notes">${escape(item.description)}</p>`:''}${item.notes?`<p class="notes"><b>Notes:</b> ${escape(item.notes)}</p>`:''}</section></article>`;}).join('');
 ...
 </style></head><body><section class="cover-page">${brand?`<img class="mark" src="${brand}" alt="GameAtlas icon">`:''}<h1>Game<em>Atlas</em></h1><p>${escape(subtitle)}</p><div class="summary"><div><b>${consoles.length}</b><span>Consoles</span></div><div><b>${peripherals.length}</b><span>Peripherals</span></div><div><b>${games.length}</b><span>Total games</span></div><div><b>${owned}</b><span>Owned games</span></div><div><b>${scored}</b><span>Scored games</span></div></div><p class="date">Exported ${escape(created.toLocaleDateString(undefined,{year:'numeric',month:'long',day:'numeric'}))}</p></section><main class="catalog">${hardware.length?`<h1 class="catalog-title">Consoles &amp; peripherals</h1>${hardwareCards}`:''}<h1 class="catalog-title">Your games</h1>${cards}</main></body></html>`;
}
