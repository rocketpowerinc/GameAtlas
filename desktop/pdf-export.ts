...
import {hardwareTypeLabels,hardwareTypes,preferredSourceUrl,type Game,type Library} from '../lib/library';
...
export function collectionPdfHtml(library:Library,directory:string,created=new Date(),subtitle='Entire Library Catalog'){
 ...
 const games=[...library.games].sort((a,b)=>year(text(a,'Release Date')).localeCompare(year(text(b,'Release Date')),undefined,{numeric:true})||text(a,'Title').localeCompare(text(b,'Title'),undefined,{sensitivity:'base'}));
 const hardware=[...(library.hardware??[])].sort((a,b)=>hardwareTypes.indexOf(a.type)-hardwareTypes.indexOf(b.type)||year(a.releaseDate).localeCompare(year(b.releaseDate),undefined,{numeric:true})||a.name.localeCompare(b.name,undefined,{sensitivity:'base'})||(a.model??'').localeCompare(b.model??'',undefined,{sensitivity:'base'}));
 const consoles=hardware.filter(item=>item.type==='Console'),unitCount=(items:typeof hardware)=>items.reduce((total,item)=>total+(item.quantity??1),0);
 const hardwareSections=hardwareTypes.map(type=>({type,title:hardwareTypeLabels[type],items:hardware.filter(item=>item.type===type)})).filter(section=>section.items.length);
 ...
 const gameCard=(game:Game,index:number)=>{
