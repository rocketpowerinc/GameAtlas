...
export function collectionPdfHtml(library:Library,directory:string,created=new Date()){
 ...
 </style></head><body><section class="cover-page"><div class="mark">GA</div><h1>Game<em>Atlas</em></h1><p>Complete Collection Catalog</p><div class="summary"><div><b>${games.length}</b><span>Total games</span></div><div><b>${owned}</b><span>Owned games</span></div><div><b>${scored}</b><span>Scored games</span></div></div><p class="date">Exported ${escape(created.toLocaleDateString(undefined,{year:'numeric',month:'long',day:'numeric'}))}</p></section><main class="catalog"><h1 class="catalog-title">Your games</h1>${cards}</main></body></html>`;
}

export async function exportCollectionPdf(library:Library,directory:string,destination:string){
 const temp=join(directory,'pdf-export-'+randomUUID()+'.html');let pdfWindow:BrowserWindow|undefined;
 try{
  writeFileSync(temp,collectionPdfHtml(library,directory));
