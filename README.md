...
Use **Settings > Create Collection PDF** to export either the games you own physically or your entire game library. Both printable catalogs keep the cover focused on console and physical-game totals. Each non-empty hardware category and the games catalog begin on separate pages, use independent numbering, and are grouped chronologically by release year. Emulation Consoles follow Consoles, then VR, Controllers, Mobile, and the remaining hardware sections, with Headphones immediately before Misc. Hardware entries include cached artwork, manufacturer, model, quantity, release date, and description. Game entries retain their pertinent details, clickable website links, and page numbers. Private Notes remain in GameAtlas and complete backups but are omitted from PDFs.
...
Physical games have a multi-select **Condition** property with Mint, Cartridge/Disc Only, Reproduction, Limited Run, and Special Edition. Condition appears only while Physical ownership is selected; removing Physical ownership clears its values. Reproduction explains that it means an unofficial reproduction cartridge or disc. Selected values appear as compact badges on the game page and as pertinent information in collection PDFs. The library filter offers every Condition choice plus Condition: Any and Condition: None; None lists physical games that still need Condition information.
