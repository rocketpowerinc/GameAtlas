...
try{
 ...
 edited.hardware=[{id:'hardware-1',name:'Test console',type:'Console',manufacturer:'Test maker',notes:'Owned'}];
 ...
 store.close();store=new LibraryStore(dir,resolve('scripts/fixtures/library.json'));assert.equal(store.read().games[0].values.Title,'Persistence test');assert.deepEqual(store.read().hardware,edited.hardware);
 ...
 console.log('PASS: game and hardware migration, persistence, revision conflicts, validation, automatic backup, restore.');
