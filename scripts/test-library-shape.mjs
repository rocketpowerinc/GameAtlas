...
const original={revision:4,fields:[
 ...
 {id:'status',name:'Status',type:'multi_select',options:['Complete','Replay']},
 {id:'notes',name:'Notes',type:'text',options:[]},
],games:[
 {id:'one',values:{title:'Example','legacy-link':'https://youtu.be/video','price':25,priority:['Want soon','Someday'],status:['Complete'],notes:'Replay on Steam'},dateEnd:'2027-02-03',dateIsTime:0,lookup:{description:'It&#x27;s fun &amp; polished.',sources:[{name:'Wikipedia',url:'https://en.wikipedia.org/wiki/Example'},{name:'Website',url:'https://www.pricecharting.com/game/example'}]}},
...
assert.deepEqual(next.games[0].values['Play Next On'],['Steam']);
assert.deepEqual(next.games[0].values.status,['Complete','Replay']);
assert.equal(next.games[0].values.notes,'');
assert.equal(next.games[1].values.notes,'Replay on Steam and PS5');
assert.equal(next.games[1].values['Play Next On'],undefined);
...
assert.equal(original.fields.length,6,'Migration must not mutate its input');
